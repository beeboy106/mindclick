...
/**
 * ประเมินคำตอบควิซของสมาชิกทุกคนพร้อมกัน (รองรับ Batched API Call ประสิทธิภาพสูง)
 */
export async function evaluateAllQuizAnswers(quizForm, loungeMembers) {
  const apiKey = getGeminiApiKey();

  // กรณีมี Gemini API Key ให้ประเมินผลแบบรวบยอด (Batched Request) ในครั้งเดียว เพื่อความรวดเร็วและป้องกัน Rate Limit
  if (apiKey && apiKey.length > 10) {
    const models = ["gemini-3.1-flash-lite", "gemini-3-flash-preview", "gemini-3.6-flash"];

    const membersPromptData = loungeMembers
      .map((member, idx) => {
        ...
        return `${idx + 1}. ID: ${member.id}, ชื่อ: ${member.alias}, บทบาทจริง: ${member.roleplay}, สิ่งที่พกมาจริง: ${member.preAnswers?.q1 || "ไม่ได้ระบุ"}
   เบาะแสในแชท: ${chatClues}
   คำตอบที่ผู้ใช้พิมพ์: "${userAns.itemAnswer || "ไม่ได้ระบุ"}"
   บทบาทที่ผู้ใช้ทาย: "${userAns.roleplayGuess || "ไม่ได้ระบุ"}"`;
      })
      .join("\n\n");

    const batchPrompt = `
คุณเป็น AI Game Master ตรวจข้อสอบภารกิจสืบสวนในเกม Mindclick ห้องสังสรรค์นิรนาม
จงประเมินคำตอบของผู้เล่นที่ตอบเกี่ยวกับเพื่อนร่วมกลุ่มทั้ง ${loungeMembers.length} คน โดยวิเคราะห์ตรรกะและเหตุผล

ข้อมูลเพื่อน เบาะแส และคำตอบของผู้ใช้:
${membersPromptData}

เกณฑ์การประเมิน:
1. ห้ามหักคะแนนจากการสะกดคำผิด คำตกหล่น หรือภาษาพูด ตราบใดที่สื่อถึงสิ่งเดียวกัน
2. ส่วนที่ 1 สิ่งที่พกมา (itemScore 0 - 25 คะแนน):
   - ตรงเป๊ะหรือความหมายตรงกัน: 25 คะแนน (isItemCorrect = true)
   - ใกล้เคียงหรืออ้างอิงเบาะแสในแชทอย่างสมเหตุสมผล: 15 - 24 คะแนน (isItemCorrect = true)
   - ไม่ตรงเลยแต่อธิบายมีเหตุผล: 5 - 14 คะแนน (isItemCorrect = false)
   - ว่างหรือไม่เกี่ยวข้อง: 0 คะแนน (isItemCorrect = false)
3. ส่วนที่ 2 บทบาทโรลเพลย์ (roleScore 0 - 25 คะแนน):
   - ทายถูกตรงกับบทบาทจริง: 25 คะแนน (isRoleCorrect = true)
   - ทายไม่ถูก: 0 คะแนน (isRoleCorrect = false)
4. กำหนด accuracyLevel: "EXCELLENT" (40-50 คะแนน), "GOOD" (25-39 คะแนน), "PARTIAL" (15-24 คะแนน), "MISSED" (0-14 คะแนน)
5. เขียน reasoning (อธิบายตรรกะ) และ feedback (คำแนะนำเชิงบวก) เป็นภาษาไทยกระชับ ชัดเจน
6. กฎเหล็ก: ห้ามใส่อีโมจิใดๆ ในข้อความผลลัพธ์โดยเด็ดขาด

ส่งคำตอบเป็น JSON ในรูปแบบนี้เท่านั้น:
{
  "evaluations": [
    {
      "memberId": "member_med",
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (evalList.length > 0) {
            const memberEvaluations = loungeMembers.map((member) => {
              const userAns = quizForm[member.id] || {};
              const matchedEval = evalList.find((e) => e.memberId === member.id);

              if (matchedEval) {
                const itemScore = Math.min(25, Math.max(0, parseInt(matchedEval.itemScore, 10) || 0));
                const roleScore = Math.min(25, Math.max(0, parseInt(matchedEval.roleScore, 10) || 0));
                const totalPoints = itemScore + roleScore;
                ...
                return {
                  ...
                  isRoleCorrect: Boolean(matchedEval.isRoleCorrect ?? roleScore >= 20),
                  isItemCorrect: Boolean(matchedEval.isItemCorrect ?? itemScore >= 15),
                  points: totalPoints,
                  itemScore,
                  roleScore,
                  accuracyLevel: matchedEval.accuracyLevel || (totalPoints >= 40 ? "EXCELLENT" : "GOOD"),
                  reasoning: matchedEval.reasoning || "ประเมินผลสำเร็จจากตรรกะและเหตุผล",
                  feedback: matchedEval.feedback || "ยอดเยี่ยมมาก คุณเป็นผู้ฟังที่ดี",
                  isAiEvaluated: true,
                  evaluatorEngine: `Google Gemini AI (${model})`,
                };
              }

              // กรณีข้อมูลสมาชิกคนนี้ไม่พบในผลลัพธ์ของ AI ให้ใช้ Fallback เฉพาะบุคคลนั้น
              return {
                ...evaluateSemanticFallback({
                  ...
                  chatClues: [...(member.chatLinesStage1 || []), ...(member.chatLinesStage2 || [])],
        ...
        console.warn(`Batched Gemini evaluation failed with model ${model}:`, err?.message || err);
      }
    }
  }

  // Fallback ประเมินผลในเครื่อง (Offline / Rate Limit Exceeded)
  const memberEvaluations = loungeMembers.map((member) => {
    const userAns = quizForm[member.id] || {};
    const chatClues = [
      ...(member.chatLinesStage1 || []),
    ...
    const result = evaluateSemanticFallback({
      memberId: member.id,
      memberAlias: member.alias,
      actualRole: member.roleplay,
      actualItem: member.preAnswers?.q1 || "",
